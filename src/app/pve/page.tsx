...
const PveTable = ({ records, onRowClick, onEditClick }: { records: PveRecord[], onRowClick: (record: PveRecord) => void, onEditClick: (record: PveRecord) => void }) => {
    return (
            ...
            <Table>
                ...
                <TableBody>
                    {records.length > 0 ? records.map(record => (
                        <TableRow key={record.id} onClick={() => onRowClick(record)} className="cursor-pointer">
                            ...
                            <TableCell>
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={() => onRowClick(record)}>Ver Detalles</DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => onEditClick(record)}>Gestionar</DropdownMenuItem>
                                  </DropdownMenuContent>
                                </DropdownMenu>
                            </TableCell>
                        </TableRow>
...
export default function PvePage() {
    ...
    const [activeTab, setActiveTab] = useState(pvePrograms[0].key);
    ...
    const filteredRecords = useMemo(() => {
        const pveKey = activeTab as keyof PveRecord;
    ...
    }, [allEmos, activeTab, filterSearch, filterCargo, filterSede, filterNivelRiesgo, user]);
    ...
    const handleSoporteUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsUploading(true);
        try {
            const downloadURL = await uploadFile(file, 'pve-soportes');
    ...
    const handleDownloadData = () => {
        ...
        link.setAttribute("download", `export_pve_${activeTab}.csv`);
    ...
    return (
        <AppLayout pageTitle="Programas de Vigilancia Epidemiológica (PVE)">
            <Card>
                <CardHeader>
                    <div className="flex flex-row items-start justify-between">
                        <div>
                            <CardTitle>Gestión de PVE</CardTitle>
                            <CardDescription>
                                Seguimiento de empleados incluidos en los diferentes programas de vigilancia epidemiológica.
                            </CardDescription>
                        </div>
                ...
                <CardContent>
                    <Tabs defaultValue={pvePrograms[0].key} onValueChange={setActiveTab}>
                        <TabsList>
                            {pvePrograms.map(pve => (
                                <TabsTrigger key={pve.key} value={pve.key}>{pve.name}</TabsTrigger>
                            ))}
                        </TabsList>
                        <Collapsible className="space-y-4 my-4">
                            <CollapsibleTrigger asChild>
                                <Button variant="outline" size="sm" className="gap-1">
                                    <Filter className="h-4 w-4"/>
                                    Filtrar Registros PVE
                                </Button>
                            </CollapsibleTrigger>
                            <CollapsibleContent>
                                <div className="p-4 border rounded-lg bg-muted/50 space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                        <div>
                                            <Label htmlFor="filter-search">Nombre o Cédula</Label>
                                            <Input id="filter-search" placeholder="Buscar..." value={filterSearch} onChange={(e) => setFilterSearch(e.target.value)} />
                                        </div>
                                        <div>
                                            <Label htmlFor="filter-cargo">Cargo</Label>
                                            <Input id="filter-cargo" placeholder="Filtrar por cargo..." value={filterCargo} onChange={(e) => setFilterCargo(e.target.value)} />
                                        </div>
                                        <div>
                                            <Label htmlFor="filter-sede">Sede/Centro de Costo</Label>
                                            <Input id="filter-sede" placeholder="Filtrar por sede..." value={filterSede} onChange={(e) => setFilterSede(e.target.value)} />
                                        </div>
                                        <div>
                        ...
                        {pvePrograms.map(pve => (
                            <TabsContent key={pve.key} value={pve.key} className="mt-4">
                                <PveTable 
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent className="sm:max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>Gestionar Seguimiento PVE</DialogTitle>
                    ...
                    <form onSubmit={handleSave} className="space-y-4 py-4 max-h-[70vh] overflow-y-auto pr-6">
                        <div className="grid grid-cols-2 gap-4">
                           <div>
                                ...
                                <Input readOnly value={selectedRecord?.tipoDocumento || ''} />
                            </div>
                            <div>
                                <Label>Identificación</Label>
                                <Input readOnly value={selectedRecord?.cedula || ''} />
                            </div>
                            <div className="col-span-2">
                                <Label>Nombre Completo</Label>
                                <Input readOnly value={selectedRecord?.nombreCompleto || ''} />
                            </div>
                             <div>
                                <Label>Cargo</Label>
                                <Input readOnly value={selectedRecord?.cargo || ''} />
                            </div>
                            <div>
                                <Label>Sede/Centro de Costo</Label>
                                <Input readOnly value={selectedRecord?.sede || ''} />
                            </div>
                             <div>
                                <Label>Fecha Ingreso al Programa</Label>
                                <Input readOnly value={formatDate(selectedRecord?.fechaExamen)} />
                            </div>
                             <div>
                                <Label>Fecha Examen Médico</Label>
                                <Input readOnly value={formatDate(selectedRecord?.fechaExamen)} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             <div>
                                ...
                                <Input id="tipoCaso" value={formData.tipoCaso} onChange={handleInputChange} />
                            </div>
                             <div>
                                <Label htmlFor="tipoPatologia">Tipo de Patología</Label>
                                <Input id="tipoPatologia" value={formData.tipoPatologia} onChange={handleInputChange} />
                            </div>
                             <div>
                             ...
                             <div>
                                ...
                                <Input id="periodicidad" value={formData.periodicidad} onChange={handleInputChange} />
                            </div>
                             <div className="col-span-2">
                                <Label htmlFor="recomendacionesColaborador">Recomendaciones para el Colaborador</Label>
                                <Textarea id="recomendacionesColaborador" value={formData.recomendacionesColaborador} onChange={handleInputChange} />
                            </div>
                             <div className="col-span-2">
                                <Label htmlFor="seguimientoRecomendaciones">Seguimiento de Recomendaciones</Label>
                                <Textarea id="seguimientoRecomendaciones" value={formData.seguimientoRecomendaciones} onChange={handleInputChange} />
                            </div>
                            <div className="col-span-2">
    ...
    return (
        <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
                ...
                <DialogDescription>
                    Información del registro PVE para {record.nombreCompleto}.
                </DialogDescription>
            </DialogHeader>
            <div className="space-y-6 py-4 max-h-[70vh] overflow-y-auto pr-6">
                <div className="space-y-4">
                    <h3 className="font-semibold text-lg border-b pb-2">Información del Empleado</h3>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <DetailItem label="Fecha Examen" value={formatDate(record.fechaExamen)} />
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <h3 className="font-semibold text-lg border-b pb-2">Datos del Seguimiento PVE</h3>
                     <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2">
                           <DetailItem label="Recomendaciones para el Colaborador" value={<p className="whitespace-pre-wrap">{record.recomendacionesColaborador}</p>} />
                        </div>
                         <div className="col-span-2">
                           <DetailItem label="Seguimiento de Recomendaciones" value={<p className="whitespace-pre-wrap">{record.seguimientoRecomendaciones}</p>} />
                        </div>
                        <div className="col-span-2">
                           <DetailItem label="Observaciones" value={<p className="whitespace-pre-wrap">{record.observaciones}</p>} />
                        </div>
                        <div className="col-span-2">
